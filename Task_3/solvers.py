...
class verlet_solvers(ui_for_particles.Ui_MainWindow):
        ...
        for t in range(1, self.M):
            ...
            for p1 in self.list_of_particles:
                for p2 in self.list_of_particles:
                    if (math.sqrt((p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2 + (p2.z - p1.z) ** 2) > 0) & (
            ...
            for j in range(n_):
                ...
                for k in range(n_):
                    if k != j:
                        ...
                        ax_ += m[k] * G * (x_[k] - x_[j]) / math.sqrt(
                            (x_[k] - x_[j]) ** 2 + (y_[k] - y_[j]) ** 2 + (z_[k] - z_[j]) ** 2) ** 3
                        ay_ += m[k] * G * (y_[k] - y_[j]) / math.sqrt(
                            (x_[k] - x_[j]) ** 2 + (y_[k] - y_[j]) ** 2 + (z_[k] - z_[j]) ** 2) ** 3
                        az_ += m[k] * G * (z_[k] - z_[j]) / math.sqrt(
                            (x_[k] - x_[j]) ** 2 + (y_[k] - y_[j]) ** 2 + (z_[k] - z_[j]) ** 2) ** 3
                ax_n.append(ax_)
                ay_n.append(ay_)
                az_n.append(az_)
            ...
            for j in range(n_):
                ...
                for k in range(n_):
                    if k != j:
                        ax_ += G * m[k] * (x_n1[k] - x_n1[j]) / math.sqrt(
                            (x_n1[k] - x_n1[j]) ** 2 + (y_n1[k] - y_n1[j]) ** 2 + (z_n1[k] - z_n1[j]) ** 2) ** 3
                        ay_ += G * m[k] * (y_n1[k] - y_n1[j]) / math.sqrt(
                            (x_n1[k] - x_n1[j]) ** 2 + (y_n1[k] - y_n1[j]) ** 2 + (z_n1[k] - z_n1[j]) ** 2) ** 3
                        az_ += G * m[k] * (z_n1[k] - z_n1[j]) / math.sqrt(
                            (x_n1[k] - x_n1[j]) ** 2 + (y_n1[k] - y_n1[j]) ** 2 + (z_n1[k] - z_n1[j]) ** 2) ** 3
                ax_n1.append(ax_)
                ay_n1.append(ay_)
                az_n1.append(az_)
    ...
    def for__scipy(self, init, t, n, mass):
        ...
        for j in range(n_):
            ...
            for k in range(n_):
                if k != j:
                    ax_ += mass[k] * G * (x_0[k] - x_0[j]) / math.sqrt(
                        (x_0[k] - x_0[j]) ** 2 + (y_0[k] - y_0[j]) ** 2 + (z_0[k] - z_0[j]) ** 2) ** 3
                    ay_ += mass[k] * G * (y_0[k] - y_0[j]) / math.sqrt(
                        (x_0[k] - x_0[j]) ** 2 + (y_0[k] - y_0[j]) ** 2 + (z_0[k] - z_0[j]) ** 2) ** 3
                    az_ += mass[k] * G * (z_0[k] - z_0[j]) / math.sqrt(
                        (x_0[k] - x_0[j]) ** 2 + (y_0[k] - y_0[j]) ** 2 + (z_0[k] - z_0[j]) ** 2) ** 3
            result.append(ax_)
            result.append(ay_)
            result.append(az_)
    ...
    def verlet__scipy(self):
        ...
        for p1 in self.list_of_particles:
            for p2 in self.list_of_particles:
                if (math.sqrt((p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2 + (p2.z - p1.z) ** 2) > 0) & (
                        math.sqrt((p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2 + (p2.z - p1.z) ** 2) < (p1.m + p2.m) / 100):
                    if p1.m > p2.m:
                        p1.m += p2.m
                    else:
                        p1.is_active = False
    ...
    class MyThread(threading.Thread):
        ...
        def run(self):
            for i in range(1, self.M_):
                ...
                for f in range(self.n):
                    if f != self.j:
                        ax += self.m[f] * self.G * (self.x[i * self.n + f] - self.x[i * self.n + self.j]) / \
                              math.sqrt((self.x[i * self.n + f] - self.x[i * self.n + self.j]) ** 2 + (
                                          self.y[i * self.n + f] - self.y[i * self.n + self.j]) ** 2 + (
                                                    self.z[i * self.n + f] - self.z[i * self.n + self.j]) ** 2) ** 3
                ...
                self.vx[i * self.n + self.j] = self.vx[(i - 1) * self.n + self.j] + 1.0 / 2 * self.dt * (
                            self.axm[i * self.n + self.j] + self.axm[(i - 1) * self.n + self.j])
                self.vy[i * self.n + self.j] = self.vy[(i - 1) * self.n + self.j] + 1.0 / 2 * self.dt * (
                            self.aym[i * self.n + self.j] + self.aym[(i - 1) * self.n + self.j])
                self.vz[i * self.n + self.j] = self.vz[(i - 1) * self.n + self.j] + 1.0 / 2 * self.dt * (
                            self.azm[i * self.n + self.j] + self.azm[(i - 1) * self.n + self.j])
            self.bar2.wait()
        ...
        for j in range(n_):
            ...
            for k in range(n_):
                if k != j:
                    ...
                    ax_ += mass[k] * G * (x[k] - x[j]) / math.sqrt(
                        (x[k] - x[j]) ** 2 + (y[k] - y[j]) ** 2 + (z[k] - z[j]) ** 2) ** 3
                    ay_ += mass[k] * G * (y[k] - y[j]) / math.sqrt(
                        (x[k] - x[j]) ** 2 + (y[k] - y[j]) ** 2 + (z[k] - z[j]) ** 2) ** 3
                    az_ += mass[k] * G * (z[k] - z[j]) / math.sqrt(
                        (x[k] - x[j]) ** 2 + (y[k] - y[j]) ** 2 + (z[k] - z[j]) ** 2) ** 3
            axm[j] = ax_
            aym[j] = ay_
            azm[j] = az_
    ...
    def verlet_cython(self):
        ...
        import pyximport
        import subprocess
        subprocess.call(["cython", "-a", "cythv.pyx"])
    ...
    def verlet_opencl(self):
        ...
        for j in range(n_):
            ...
            for f in range(n_):
                if f != j:
                    ax += mass[f] * G * (x[f] - x[j]) / math.sqrt(
                        (x[f] - x[j]) ** 2 + (y[f] - y[j]) ** 2 + (z[f] - z[j]) ** 2) ** 3
                    ay += mass[f] * G * (y[f] - y[j]) / math.sqrt(
                        (x[f] - x[j]) ** 2 + (y[f] - y[j]) ** 2 + (z[f] - z[j]) ** 2) ** 3
                    az += mass[f] * G * (y[f] - y[j]) / math.sqrt(
                        (x[f] - x[j]) ** 2 + (y[f] - y[j]) ** 2 + (z[f] - z[j]) ** 2) ** 3
            axm[j] = ax
            aym[j] = ay
            azm[j] = az
        ...
        prg.compute(queue, (1,), None, lengthn, lengtht, Gb, dtb, mbuf, xbuf, ybuf, zbuf, vxbuf, vybuf, vzbuf, axmbuf,
                    aymbuf, azmbuf).wait()
        computing_time = time.clock() - start_time
        print(time.clock() - start_time)
        self.gui.label_time.setText(str(computing_time))
